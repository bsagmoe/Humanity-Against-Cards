...
class HumanityAgainstCards {
  constructor(gameSettings, gameId, gameName, deck) {
    this.gameId = gameId;
  ...
  startGame(){
    if(this.gameStatus != GAME_STATUS.INITIALIZING){
      ...
      let handlers = this.registeredEventHandlers[GAME_EVENTS.GAME_STARTED_EVENT];
      if (handlers != null) {
          for (let i = 0; i < handlers.length; i++) {
              handlers[i](this.roundInfo, this.gameStats);
          }
  ...
  signalNotReadyForSubmission(player){
    for(let i = 0; i < this.players.length; i++){
      if(player.id == this.players[i].id){
  ...
  judge(winningCard){
    ...
    this.cardCzar.isCardCzar = false;  // make the current Czar a Prole
    this.cardCzar = roundWinner;
    this.cardCzar.isCardCzar = true;     // and make the round winner the Czar
    this.roundInfo.updateCardCzar(this.cardCzar);
...
module.exports = {
  ...
  RoundInfo: RoundInfo,
  Player: Player,
  HumanityAgainstCards: HumanityAgainstCards
};
